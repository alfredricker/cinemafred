...
class HLSManager {
  constructor(config) {
    ...
    this.failedSegmentAttempts = /* @__PURE__ */ new Map();
    this.maxRetries = 3;
    this.maxFailedSegmentAttempts = 2;
    this.failedSegmentCooldownMs = 2 * 60 * 1e3;
    this.segmentSkipLeadSeconds = 5;
    this.segmentSkipBufferSeconds = 0.5;
    this.postSkipSettleMs = 3e3;
    this.retryCount = 0;
  ...
  handleError(event, data) {
    ...
    const isBufferAppendingError = data.type === ErrorTypes.MEDIA_ERROR && (details === "bufferAppendingError" || details === "bufferAppendError");
    if ((isParsingError || isBufferAppendingError || isNetworkError && data.fatal) && data.frag) {
      ...
      if (isBufferAppendingError && this.hls) {
        const now = Date.now();
        if (now - this.lastMediaRecoveryAt > 1e3) {
  ...
  computeChainedJumpTarget(ranges, initialEnd) {
  ...
  trySkipBlockedSegmentNow() {
    ...
    for (const range of activeRanges) {
      ...
      const isInsideBlockedRange = currentTime >= range.start - this.segmentSkipBufferSeconds && currentTime <= range.end + this.segmentSkipBufferSeconds;
      const isApproachingBlockedRange = distanceToSegment > -this.segmentSkipBufferSeconds && distanceToSegment <= this.segmentSkipLeadSeconds;
