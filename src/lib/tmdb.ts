...
interface TMDBMovieCredit {
  ...
  private async getMovieDetails(movieId: number, page: number = 1, postersPerPage: number = 8): Promise<MovieMetadata> {
    ...
    try {
      ...
      if (imagesResponse.ok) {
        ...
        // Get posters based on page and postersPerPage, prioritize English language posters
  ...
  async searchMovie(input: string, page: number = 1, postersPerPage: number = 8): Promise<{ metadata: MovieMetadata | null, suggestions?: MovieSuggestion[] }> {
    try {
      let title: string;
      let year: number;
      ...
      console.log('Searching TMDB with:', { title, year, page });
      ...
      const bestMatch = await this.getMovieDetails(scoredResults[0].id, page, postersPerPage);
